...
- `example/`: sample app in a separate Go module, included via `go.work`.
...
- Release builds: `cd /home/krishndu/go/c2pa-go && go build -tags=release ./c2pa && cd example && go build -tags=release .`
...
- When validating a change, start with `./c2pa` rather than `./...` unless the example module was also touched.
